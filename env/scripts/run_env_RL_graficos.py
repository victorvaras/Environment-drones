...
def make_timing_plot_with_config(
    ...
    Genera PNG con:
      ...
      - leyenda abajo (fuera) con promedios y totales,
      - incluye explícitamente Step 0 y total sin Step 0.
    ...
    # --- Estadísticos ---
    step0 = float(dt[0]) if dt.size > 0 else float("nan")
    ...
    total_wo0 = float(np.sum(dt[1:])) if dt.size > 1 else float("nan")
    ...
    # Entradas “de texto” (sin línea)
    text_entries = []

    if np.isfinite(step0):
        text_entries.append(Line2D([], [], linestyle="None",
                                   label=f"Step 0: {step0:.6f} s"))
    ...
    if np.isfinite(cum_final):
        text_entries.append(Line2D([], [], linestyle="None",
                                   label=f"Tiempo acumulado final (incluye step 0): {cum_final:.6f} s"))

    if np.isfinite(total_wo0):
        text_entries.append(Line2D([], [], linestyle="None",
                                   label=f"Tiempo total (sin step 0, desde step 1): {total_wo0:.6f} s"))
    ...
    # Espacio inferior para la leyenda externa
    ...
    # Retorna también total sin step 0 por si lo quieres imprimir afuera
    return str(out_path), avg_from0, avg_from1, total_wo0
...
if __name__ == "__main__":
    ...
    # Opcional: reduce “picos” por GC durante el loop
    gc.disable()

    # -----------------------------
    # 1) Reset (medido aparte)
    # -----------------------------
    t_reset0 = perf_counter()
    obs, info = env.reset(seed=0)
    t_reset = perf_counter() - t_reset0
    ...
    # -----------------------------
    # 2) Episodio: medir TODOS los steps (incluye step 0)
    # -----------------------------
    ...
    step0 = float(dt[0]) if n > 0 else float("nan")
    total_all = float(dt.sum()) if n > 0 else float("nan")
    total_wo0 = float(dt[1:].sum()) if n > 1 else float("nan")
    avg_all = float(dt.mean()) if n > 0 else float("nan")
    avg_wo0 = float(dt[1:].mean()) if n > 1 else float("nan")

    print(f"Reset time: {t_reset:.6f} s")
    print(f"Loop wall-clock (mientras ejecuta steps): {t_loop:.6f} s")
    print(f"Steps ejecutados: {n}")

    if np.isfinite(step0):
        print(f"Step 0: {step0:.6f} s")

    if np.isfinite(total_all):
        print(f"Total (incluye step 0): {total_all:.6f} s")

    if np.isfinite(total_wo0):
        print(f"Total (sin step 0, desde step 1): {total_wo0:.6f} s")

    if np.isfinite(avg_all):
        print(f"Promedio (desde step 0): {avg_all:.6f} s")

    if np.isfinite(avg_wo0):
        print(f"Promedio (desde step 1): {avg_wo0:.6f} s")

    # -----------------------------
    # 4) RT params actuales
    # -----------------------------
    rt_params = {
    ...
    # -----------------------------
    # 5) Plot (medido aparte)
    # -----------------------------
    t_plot0 = perf_counter()
    png_path, avg0_plot, avg1_plot, total_wo0_plot = make_timing_plot_with_config(
        dt_list,
        rt_params,
        out_dir=OUT_DIR,
    ...
    t_plot = perf_counter() - t_plot0

    print("PNG guardado en:", png_path)
    print(f"Tiempo plot+save PNG: {t_plot:.6f} s")
